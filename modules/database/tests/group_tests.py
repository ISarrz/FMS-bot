...
def test_parent_fetch():
    parent = Group.insert(name="parent")
    group1 = Group.insert(name="group")
    group2 = Group.insert(name="group")
    parent.insert_child(group1)
    group = Group(name="group", parent=parent)
    assert group.parent.id == parent.id
